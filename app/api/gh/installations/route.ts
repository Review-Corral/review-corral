...
export const GET = withAxiom(async (req: AxiomRequest) => {
  ...
  if (!user) {
    req.log.error("No user found for id", { id: data.session.user.id });
  ...
  if (organizations.data !== null) {
    for (const installation of reponse.data.installations) {
      ...
      if (
        ...
        if (error) {
          req.log.error(`Error updating org installation id: ${error.message}`);
  ...
  NextResponse.json(reponse.data);
});
